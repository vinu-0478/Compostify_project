...
function Searchnearby() {
  ...
  async function handleChangeType(e) {
    ...
    try {
      ...
      console.log(response.data.nearbyAgency);
      setDataList(response.data.nearbyAgency);
    } catch (err) {
      if (err.response && err.response.status === 422) {
        alert("Your location is not updated. Please update your profile.");
        redirect("/profile");
      } else if (err.response && err.response.status === 403) {
        alert("You are not authorized to access this resource. Please log in.");
        redirect("/login");
      } else {
        console.error(err);
      }
